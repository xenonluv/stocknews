...
def _next_bar(code, signal_date):
    """signal_date 다음 거래일 일봉(J 공식) | None. 정지봉(close==0) 제외."""
    try:
        d = kis.daily_prices(code, days=8, market="J")
    except Exception:
        return None
    after = [x for x in d if x.get("date") and x["date"] > signal_date and (x.get("close") or 0) > 0]
