...
- `/alpha` — **알파 전진검증** 페이지(SSG + 60초 폴링). 데이터 = `alpha.json`. mover 카드(유통회전율 2일·종가강도·14:30스파크·
  ...
  - **종가베팅 적합도 정렬·색**(웹 표시만, alpha.json 무수정 — `closeBetFitness`/`fitnessTier`): 카드 정렬=**종가베팅 적합도 점수(0~100, 잠정 휴리스틱) 내림차순**, 동점은 2일회전율이 스윗스팟 중심(115%)에 가까운 순. 점수=기준 50 + 가감: **유형**(reaccum +10 / youtong 0 / explosion −45=하위로) + **2일회전율**(80~150% +15 / 40~80 +8 / <40 +3 / 150~250 −10 / 250%+ −5) + **당일등락**(0~+8% +15 / ≤−10% 깊은눌림 +12 / −10~0 +3 / +8~+15 −20 / +15~+22 −30 / +22%+ −40 — 많이 오를수록 단조 강등: 이미 오른 종목은 종가 추격 매수 불가·익일 갭 위험, 회장님 지시 2026-07-01) + **14:30스파크**(1~2회 +12 / 0회 +2 / 3회+ −8 과열 / 미측정 0) + **숨은외인매집**(lv≥1 −5 역신호). 색=한국관례(빨강=좋음) 75+적합·빨강굵게/60+중간·주황/45+약·앰버/그외 부적합·회색. 카드에 **순위 배지(N위·1위 빨강~2위 주황~3위 앰버~하위 회색) + 유형 라벨(재매집/후보/폭발) + 종베 점수 배지 + 가점/감점 근거 칩** 노출(투명 공개). ⚠ **2026-06-30 회장님 종가베팅 전략 분석(22표본·2거래일) 근거 — 잠정·표시 전용, calibration 패널(`combined_score` 검증)과는 별개 축.** explosion·과열·극단회전 종목이 하위로 밀려 "사도 되는 종목"이 상위에 옴(구 정렬 `combined_score`=스파크+외인 큰 순은 식을 종목을 상위로 올리던 역신호라 폐기).
