...
| `publish.py` | radar → `web/data/radar.json` → 변경 시에만 commit+push. flock 락, `--dry-run`, `--max`. radar 인자 그대로 전달. `explosions[]`·`youtong[]`은 radar.py 출력 그대로 패스스루. 매 회차 `data/radar_history/`에 검증용 이력(raw 점수) 기록. 게시 후보 중 **재반등 봉 발생 시 `telegram_notify.notify_reignitions` 호출**(토큰 없으면 조용히 skip). |
