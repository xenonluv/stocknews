...
레이더는 **하나의 흐름(폭발→식음→반등)** + **당일 폭발 리스트**를 게시한다. 모든 reaccum/explosion
...
- **식음(fade)** — 폭발 후 윈도(기본 6거래일) 내 **고점(폭발일 이후 일봉 최고가) 대비 −15%~−40% 폭락**한 상태.
- **반등조짐(reaccum 수상종목)** — 식음 상태에서 **당일 15분봉 양봉(몸통%≥2%)이 2회 이상**.
  ⚠️ 등락률 게이트·MA20 생존·투신(ivtr) 매집·거래원·거래대금 게이트는 **전부 폐지**(순수 차트 게이트).
...
[정밀 판정·종목별, KIS 공식 API]
   식음: daily(고점=폭발일 이후 최고가) → 현재가 고점 대비 −15%~−40%
   반등: minute_bars_today → 15분봉 양봉(몸통%≥2%) 2회 이상(reignition_bars)
   ▼
[조건 가점] event_calendar(D-10 정적 캘린더+규칙) × theme_map(뉴스·업종 테마 매칭)
   ▼
[점수] 반등조짐 변별 점수 = base62 + drawdown(0~10, 식음 깊이)+re_count(0~8, 15분 양봉 수)
...
| `telegram_notify.py` | **재반등 15분봉 알림** (봇 `@signalpyo_bot`, 표준라이브러리만). **완료된 봉만**(`_bar_complete`, 15분 경계) · 봉 단위 디둡(`date:code:HH:MM`, `.telegram_notified.json`) · fail-safe(실패해도 publish 진행). `_load_state`는 손상 파일도 빈 상태로 안전 처리. 시크릿 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`는 **Mac `.env`에만**. |
...
- **KIS 공식 API** (`openapi.koreainvestment.com:9443`, .env의 KIS_APP_KEY/SECRET):
  ...
  - ⚠️ **시장구분(`FID_COND_MRKT_DIV_CODE`) 분리 정책 — 가격=J / 거래대금·수급=UN**:
    ...
    왜곡 없음). 반등은 **15분봉 양봉 횟수**만 보고 거래대금 게이트가 없어 UN/J 스케일 보정이 불필요(개편 단순화).
...
```
1,11,21,31,41,51 9-15 * * 1-5  publish.py                 # 10분 간격, :01 오프셋(당일 폭발 + 식음·반등 게시)
...
- 재반등 봉은 15분봉이라 텔레그램 알림은 봉 완성(:15/:30/:45/:00) 후 다음 publish 회차에 전송(지연 ≤~10분).
...
- `GET /api/radar` — 레이더 전체 상태 `{generated_at, market_session, events[], explosions[], suspects[], params}`.
  ...
  `drawdown_pct`·`reignition`(15분봉·count)·`forecast` 포함.
