...
- **곧 폭발할 후보(youtong)** — 당일 **현재 등락률 ≥10% AND 유통주식 회전율 70~100%**(폭발 직전·진행중 매집),
  단 **이미 폭발(고가≥22% AND 회전율≥90%)한 종목은 제외**(=/forecast로 분류, 역할 분리). **`/youtong`에 게시.**
  같은 네이버 up 랭킹 스캔에서 수집(float 조회 게이트를 `high≥22 OR change≥10`으로 공유 — price_now는 이미
  전 행 조회, float은 7일 캐시 완충). **라이브 스냅샷 전용**(registry 미적재·백필 없음 — 밀려나면 빠지는 게 맞음).
  임계: `--youtong-change-pct`(10) `--youtong-turnover-min`(70) `--youtong-turnover-max`(100). 표시·참고용(통계 무관).
...
[폭발 캐치] 고가등락률 ≥22% AND 당일 거래량/유통주식수 ≥90% → registry(.explosion_registry.json)
   ...
   ▼ (같은 스캔 루프에서 동시 수집)
[곧 폭발 후보] 현재 등락률 ≥10% AND 유통회전율 70~100% AND 미폭발 → youtong[](/youtong 게시). 라이브 스냅샷 전용.
   ▼
...
| `telegram_notify.py` | **텔레그램 알림** (봇 `@signalpyo_bot`, 표준라이브러리만). 두 종류, 메시지·디둡파일 분리로 한 채팅서 구분: ① **재매집 5분 스파크**(`notify_reignitions`) — "🚨 …재반등 봉", **완료된 봉만**(`_bar_complete`, span_min=publish가 radar params로 전달, 기본 5분 경계)·봉 단위 디둡(`date:code:HH:MM`, `.telegram_notified.json`). ② **곧 폭발 후보(youtong)**(`notify_youtong`) — "⚡ …곧 폭발 후보"(현재 등락률·유통 회전율·거래대금), **종목·일자 1회 디둡**(code 키, `.youtong_notified.json`), 봉 완성 판정 없이 밴드 첫 진입 시 1통(라이브 스냅샷 도배 방지). 둘 다 fail-safe(실패해도 publish 진행)·`send`/`load_env`/`_load_state` 공용. `_load_state`는 손상 파일도 빈 상태로 안전 처리. 시크릿 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`는 **Mac `.env`에만**. |
...
- `/youtong` — **곧 폭발할 후보** 페이지(SSG + 60초 폴링). 데이터 = `radar.json`의 `youtong[]`(현재 등락률≥10% AND
  ...
  youtong 밴드 첫 진입 시 **텔레그램 알림**("⚡ …곧 폭발 후보", `telegram_notify.notify_youtong`, 종목·일자 1회 디둡 — Mac만).
...
- `.kis_token.json`·`.telegram_notified.json`·`.youtong_notified.json`·`open_api/`·`apikey.md`·`kimiapi.md`·`kis_devlp.yaml` 모두 gitignore.
