...
| `radar.py` | 6조건 스캐너 CLI. 유니버스 = 시장별 거래대금·등락률 TOP-N 합집합(`--top-n` 기본 20, KIS 장애 시 네이버 전수 스캔 자동 폴백). `--min-value`(원, 기본 700억 — **정밀판정 게이트**로만 적용) `--high-pct`(13) `--chg-min/max`(−6/10) `--spark-x`(분봉 거래량 중앙값 8배) `--spark-pct`(0.8) `--names`(watchlist). stdout JSON. 유니버스 0종목이면 exit 2 (수집 장애 구분). |
