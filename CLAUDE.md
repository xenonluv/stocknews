...
- `/alpha` — **알파 전진검증** 페이지(SSG + 60초 폴링). 데이터 = `alpha.json`. mover 카드(유통회전율 2일·종가강도·14:30스파크·
  ...
  + **14:30 스파크 횟수 단독** + **🕵 키움 속 외인매집(해당/미해당)** + **종합점수(스파크+외인매집)별 — 정렬 순위 검증** 실측 익일확률, **min_n 게이트**=부족 셀 "관찰중").
