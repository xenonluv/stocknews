...
radar.py 인자(--fade-drawdown-min/max --reignition-body-pct --reignition-span-min/min-count
...
RADAR_PASSTHRU = ("--fade-drawdown-min", "--fade-drawdown-max",
                  "--reignition-body-pct", "--reignition-span-min", "--reignition-min-count",
...
def record_history(out):
    ...
    for rank, s in enumerate(out.get("suspects", []), 1):
        ...
        hist["suspects"][s["code"]] = {
            ...
            "drawdown_pct": s.get("drawdown_pct"),  # 식음 깊이 — 폭발 후 고점 대비 하락률(%)
            ...
            "reignition": s.get("reignition"),  # 재반등(오늘) 신호: 15분봉 몸통%·시각·양봉수
...
def main():
    ...
    # 텔레그램: 게시 후보의 새(완성된) 재반등 15분봉마다 알림. 봉 시각 디둡(도배 방지).
    ...
    if not dry:
        try:
            span = int(radar.get("params", {}).get("reignition_span_min") or 15)
